...
export default function AdminAlmacen() {
  ...
  const [selectedIngredienteForReceta, setSelectedIngredienteForReceta] = useState('');
  ...
  async function handleAjusteStock(e) {
    e.preventDefault();
    try {
      const ingId = currentIngrediente._id || currentIngrediente.id;
      if (!ingId) {
        Swal.fire('Error', 'El ingrediente no tiene un ID válido', 'error');
        return;
      }
      await ajustarStock(ingId, ajusteData.cantidad, ajusteData.tipo);
  ...
  async function openRecetaModal(type, item) {
    ...
    setCurrentRecetaTarget({ type, id: itemId, nombre: item.nombre });
    setRecetaData([]);
    setSelectedIngredienteForReceta('');
    ...
    try {
      let receta = [];
      if (type === 'producto') {
        receta = await getRecetaProducto(itemId);
      } else {
        receta = await getRecetaPersonalizacion(itemId);
      }
  ...
  function handleAddIngredienteToReceta() {
    if (!selectedIngredienteForReceta) {
      Swal.fire('Info', 'Selecciona un ingrediente', 'info');
      return;
    }
    const ing = ingredientes.find(i => i._id === selectedIngredienteForReceta || i.id === selectedIngredienteForReceta);
    if (!ing) {
      Swal.fire('Error', 'Ingrediente no encontrado', 'error');
      return;
    }
    
    const exists = recetaData.find(r => r.ingrediente_id === selectedIngredienteForReceta);
    if (exists) {
      Swal.fire('Info', 'El ingrediente ya está en la receta. Puedes modificar la cantidad en la lista.', 'info');
      return;
    }
    
    setRecetaData([...recetaData, { 
      ingrediente_id: selectedIngredienteForReceta, 
      cantidad: Number(cantidadIngrediente), 
      nombre: ing.nombre, 
      unidad_medida: ing.unidad_medida,
      categoria_reemplazo: ing.categoria_reemplazo || null
    }]);
    setSelectedIngredienteForReceta('');
    setCantidadIngrediente(1);
  }
  ...
  async function handleSaveReceta() {
    try {
      ...
      console.log('Guardando receta:', payload);
      ...
      console.log('Receta guardada, recargando...');
      
      // Recargar la receta después de guardar
      let receta = [];
      if (currentRecetaTarget.type === 'producto') {
        receta = await getRecetaProducto(currentRecetaTarget.id);
      } else {
        receta = await getRecetaPersonalizacion(currentRecetaTarget.id);
      }
      
      console.log('Receta recargada:', receta);
      setRecetaData(receta || []);
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {ingredientes.map(ing => {
                    ...
                    return (
                      <tr key={ingId} className={isBajo ? 'stock-bajo' : ''}>
            ...
            <div className="tab-section">
              ...
              <div className="recetas-grid">
                {productos.map(prod => {
                  ...
                  return (
                    <div key={prodId} className="receta-card">
            ...
            <div className="tab-section">
              ...
              <div className="recetas-grid">
                {personalizaciones.map(pers => {
                  const persId = pers._id || pers.id;
                  return (
                    <div key={persId} className="receta-card">
                      <div className="receta-info">
                        <h3>{pers.nombre}</h3>
                        <p>{pers.tipo}</p>
                      </div>
        ...
        <div className="receta-modal-content">
          <div className="receta-info-box">
            ...
            <ul style={{fontSize: '0.9rem', color: 'var(--color-text-secondary)', paddingLeft: '20px'}}>
              ...
              <li>Si un ingrediente tiene <strong>Categoría de Reemplazo</strong>, se omitirá cuando el cliente elija una personalización de esa categoría</li>
            </ul>
          </div>
          ...
          <div className="form-group mt-4">
            ...
            <div style={{display: 'flex', gap: '10px'}}>
              <select 
                ...
                value={selectedIngredienteForReceta}
                onChange={(e) => setSelectedIngredienteForReceta(e.target.value)}
              >
                <option value="">Seleccione un ingrediente...</option>
                {ingredientes.filter(i => !recetaData.find(r => r.ingrediente_id === (i._id || i.id))).map(ing => {
                  const ingId = ing._id || ing.id;
                  return (
                    <option key={ingId} value={ingId}>
                      {ing.nombre} ({ing.unidad_medida}) 
                      {ing.categoria_reemplazo && ` - ${ing.categoria_reemplazo}`}
                    </option>
                  );
                })}
              </select>
              <button type="button" className="secondary-btn" onClick={handleAddIngredienteToReceta}>
                <Plus size={18} /> Añadir
              </button>
            </div>
          </div>
                  ...
                  {recetaData.map(item => (
                    <tr key={item.ingrediente_id}>
                      <td>
                        ...
                        {item.categoria_reemplazo && (
                          <span className="badge-category" style={{marginLeft: '8px', fontSize: '0.75rem'}}>
                            {item.categoria_reemplazo}
                          </span>
                        )}
                      </td>
