...
export default function VentasVendedor() {
  ...
  async function handleRefund() {
    ...
    try {
      const ventaId = ventaSeleccionada._id || ventaSeleccionada.id;
      await refundSale(ventaId, refundPassword, refundMotivo);
  ...
  function handleCorteCaja() {
    if (cashRegister?._id || cashRegister?.id) {
            ...
            <table className="mv-table">
              ...
              <tbody>
                {filteredSales.map((venta) => {
                  ...
                  return (
                    <tr key={ventaId}>
                      <td data-label="Folio">#{venta.numero_venta || ventaId}</td>
                      <td data-label="Fecha">
                      ...
                      <td data-label="Estado">
                        <span className={`status-chip ${venta.devuelta ? 'refunded' : venta.cancelada ? 'cancelled' : 'completed'}`}>
                          {venta.devuelta ? 'Devuelta' : venta.cancelada ? 'Cancelada' : 'Completada'}
                        </span>
                      </td>
                      <td data-label="Acciones">
                        <div className="row-actions">
                          ...
                          {!venta.cancelada && !venta.devuelta && (
                            <button
                              type="button"
