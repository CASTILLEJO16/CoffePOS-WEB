...
export default function UserActivity() {
  ...
  function getFilteredActivities() {
    ...
    activity.cashRegisters?.forEach(caja => {
      activities.push({
        type: 'caja',
        id: caja._id || caja.id || caja.fecha_apertura || Math.random().toString(36).substr(2, 9),
    ...
    activity.sales?.forEach(venta => {
      activities.push({
        type: 'venta',
        id: venta._id || venta.id || venta.fecha || Math.random().toString(36).substr(2, 9),
        fecha: venta.fecha,
        estado: 'completada',
        nombre: `Venta #${venta.numero_venta || 'N/A'}`,
        monto: venta.total,
    ...
    activity.logs?.forEach(log => {
      activities.push({
        type: 'log',
        id: log._id || log.id || log.fecha || Math.random().toString(36).substr(2, 9),
        fecha: log.fecha,
        estado: 'registrado',
        nombre: log.accion,
