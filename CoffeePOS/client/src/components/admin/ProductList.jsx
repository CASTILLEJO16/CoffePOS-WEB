...
export default function ProductList({ onEdit, onRefresh }) {
  ...
  async function loadProducts() {
    try {
      ...
      console.log('Productos cargados:', data.map(p => ({ nombre: p.nombre, _id: p._id, id: p.id })));
      setProducts(data);
    } catch (error) {
  ...
  async function handleDeleteConfirm() {
    try {
      ...
      if (!productId) {
        console.error('Producto sin ID válido:', productToDelete);
        alert('Error: El producto no tiene un ID válido');
        return;
      }
        ...
        <table className="data-table">
          ...
          <tbody>
            {products.map(product => {
              ...
              return (
                <tr key={productId} className={!product.activo ? 'row-inactive' : ''} style={{ backgroundColor: isSelected ? 'var(--color-primary-light)' : ''}}>
                  <td>
                    <button
